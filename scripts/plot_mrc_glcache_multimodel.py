...
if __name__ == "__main__":
    ...
    for algo in ap.algos.split(","):
        ...
        ap.algos = "gl-cache-every-day,gl-cache-no-retrain"
        
        # glob thourh /dump folder, load model for each
        # sleep 1 second
        import time
        time.sleep(1)
        
        import glob
        folder_of_cwd = os.getcwd()
        for idx, model_file in enumerate(glob.glob(f"{folder_of_cwd}/dump/*")):
            # regex the model_n.bin in the filename
            model_ver = re.search(r"model_(\d+).bin", model_file).group(1)
            print("MODEL FILE", model_file, model_ver)
            key = algo + "-load-" + str(model_ver)
            mrc_dict[key] = run_cachesim_time_custom(
                ap.tracepath,
                algo,
                ap.size,
                ap.ignore_obj_size,
